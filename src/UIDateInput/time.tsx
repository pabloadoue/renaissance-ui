...
import { View, Text, HStack, ScrollView, Pressable } from "native-base";
...
export default function UIDateInputTimePicker(props: UIDateTimeProps) {
    ...
    const renderHours = () => {
        return hours.map((hour, index) => {
            return <View width="100%" key={index} alignItems={"flex-end"} justifyContent={"flex-end"}>
                <Pressable width={16} justifyContent={"center"} alignItems="center" onPress={() => {
                    ...
                    {(params) => {
                        ...
                        return <View width="100%" justifyContent={"center"} bg={selected ? "primary.500" : "transparent"} alignItems="center" opacity={isPressed ? .5 : 1} paddingY={1} borderRadius={4}>
                            <Text color={selected ? "white" : "gray.500"} fontSize={18}>{hour.label}</Text>
                        </View>
                    }}
                </Pressable>
            </View>
    ...
    const renderMinutes = () => {
        return minutes.map((minute, index) => {
            return <View width="100%" key={index} alignItems={"flex-start"} justifyContent={"flex-start"}>
                <Pressable width={16} justifyContent={"center"} alignItems="center" onPress={() => {
                    ...
                    {(params) => {
                        ...
                        return <View width="100%" justifyContent={"center"} bg={selected ? "primary.500" : "transparent"} alignItems="center" opacity={isPressed ? .5 : 1} paddingY={1} borderRadius={4}>
                            <Text color={selected ? "white" : "gray.500"} fontSize={18}>{minute.label}</Text>
                        </View>
                    }}
                </Pressable>
            </View>
