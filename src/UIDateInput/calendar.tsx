...
import { View, Text, Pressable, HStack, useTheme } from "native-base";
...
export default function UIDateInputCalendar(props: UIDateCalendarProps) {
    ...
    const renderMonths = () => {
        return months.map((month) => {
            return <View key={month.value} width="28%" justifyContent={"center"} alignItems="center" marginBottom={4} marginX={2}>
                <Pressable width="100%" justifyContent={"center"} alignItems="center" onPress={() => {
                    ...
                    {(params) => {
                        ...
                        return <View width="100%" justifyContent={"center"} bg={selected ? "primary.500" : "transparent"} alignItems="center" opacity={isPressed ? .5 : 1} paddingY={1} borderRadius={4}>
                            <Text color={selected ? "white" : "gray.500"}>{month.label}</Text>
                        </View>
                    }}
                </Pressable>
            </View>
    ...
    const renderYears = () => {
        return years.map((year) => {
            return <View key={year.value} width="20%" justifyContent={"center"} alignItems="center" marginBottom={4} marginX={2}>
                <Pressable width="100%" justifyContent={"center"} alignItems="center" onPress={() => {
                    ...
                    {(params) => {
                        ...
                        return <View width="100%" justifyContent={"center"} bg={selected ? "primary.500" : "transparent"} alignItems="center" opacity={isPressed ? .5 : 1} paddingY={1} borderRadius={4}>
                            <Text color={selected ? "white" : "gray.500"}>{year.label}</Text>
                        </View>
                    }}
                </Pressable>
            </View>
    ...
    const content = () => {
        if (calendarView === "calendar") {
            return <Calendar
                ...
                markedDates={{
                    [props.value.format("YYYY-MM-DD")]: { selected: true, selectedColor: colors.primary["500"] }
                }}
                onDayPress={({ dateString }) => {
                    const hour = props.value.hour();
                    const minute = props.value.minute();
                    const second = props.value.second();
                    const value = dayjs(dateString).hour(hour).minute(minute).second(second);
                    props.change(value);
                    props.close();
                }}
                renderHeader={(date) => {
                    return <View>
                        <Pressable onPress={() => {
                            ...
                            {(params) => {
                                const { isPressed } = params;
                                return <Text fontWeight={"bold"} opacity={isPressed ? 0.5 : 1} fontSize={16}>{dayjs(date).format("MMMM YYYY")}</Text>
                            }}
                        </Pressable>
                    </View>
                }}
                renderArrow={(direction) => {
                    if (direction === "left") {
                        return <UIIcon name="arrow-back-ios" size={4} color={"primary.500"} />
                    } else {
                        return <UIIcon name="arrow-forward-ios" size={4} color={"primary.500"} />
                    }
                }}
            />
        } else if (calendarView === "months") {
            return <View width="100%" paddingY={2}>
                    ...
                    <HStack width="100%" paddingX={8}>
                        <View flex={1} justifyContent={"center"} alignItems={"flex-start"}>
                            <Pressable onPress={() => {
                                ...
                                {(params) => {
                                    const { isPressed } = params;
                                    return <View opacity={isPressed ? 0.5 : 1}>
                                        <UIIcon name="arrow-back-ios" size={4} color={"primary.500"} />
                                    </View>
                                }}
                            </Pressable>
                        </View>
                        <View flex={1} justifyContent="center" alignItems={"center"}>
                            <Pressable onPress={() => {
                                ...
                                {(params) => {
                                    const { isPressed } = params;
                                    return <Text fontWeight={"bold"} opacity={isPressed ? 0.5 : 1} fontSize={16}>{props.value.format("YYYY")}</Text>
                                }}
                            </Pressable>
                        </View>
                        <View flex={1} justifyContent={"center"} alignItems={"flex-end"}>
                            <Pressable onPress={() => {
                                ...
                                {(params) => {
                                    const { isPressed } = params;
                                    return <View opacity={isPressed ? 0.5 : 1}>
                                        <UIIcon name="arrow-forward-ios" size={4} color={"primary.500"} />
                                    </View>
                                }}
                            </Pressable>
                        </View>
    ...
    return <View width="100%" alignItems={"center"}>
